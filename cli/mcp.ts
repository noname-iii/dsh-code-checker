...
const TOOLS = [  // 定义 MCP 服务器对外暴露的工具列表
  {  // 第一个工具：check_project
    ...
    inputSchema: {  // 工具输入参数 JSON Schema
      type: 'object',  // 参数类型为对象
      properties: {  // 对象属性定义
...
async function checkProject(args: Record<string, unknown>): Promise<string> {  // 执行项目检查的工具实现，返回渲染后的报告文本
  ...
  if (!existsSync(projectDir)) return '错误：项目目录不存在: ' + projectDir  // 目录不存在时直接返回错误文本
  ...
  if (baseUrl && model) analyzer = makeOpenAiAnalyzer({ baseUrl, apiKey, model })  // 地址与模型齐全时构造 LLM 分析器
  ...
  const report = await runCheck(options, {  // 执行完整检查
    exec: makeExec(platform),  // 前台执行适配器
    start: makeStart(platform),  // 后台启动适配器
    platform,  // 平台标识
    ...analyzer !== undefined ? { analyzer } : {},  // 分析器存在时携带 analyzer 字段
    log: (line: string): void => { console.error('[mcp] ' + line) },  // 日志函数：输出到标准错误
  })
  return report.rendered  // 返回渲染后的报告文本
}

async function detectProject(args: Record<string, unknown>): Promise<string> {  // 执行项目识别的工具实现，返回 JSON 字符串
  const platform: 'win32' | 'posix' = process.platform === 'win32' ? 'win32' : 'posix'  // 根据运行平台判定 win32 或 posix
  const projectDir = resolve(typeof args.project_dir === 'string' && args.project_dir ? args.project_dir : process.cwd())  // 解析项目目录（缺省当前工作目录）
  const { detectProject: runDetect } = await import('../engine/detect.js')  // 动态导入项目识别函数并重命名为 runDetect
  const info = await runDetect(projectDir, {  // 执行项目识别
    exec: makeExec(platform),  // 前台执行适配器
    platform,  // 平台标识
    log: (line: string): void => { console.error('[mcp] ' + line) },  // 日志函数：输出到标准错误
  })
...
function send(message: unknown): void {  // 向标准输出发送一条 JSON-RPC 消息（单行 JSON + 换行）
  process.stdout.write(JSON.stringify(message) + '\n')  // 序列化消息并追加换行写入 stdout
}

/** 运行 MCP stdio 服务器（阻塞直至 stdin 关闭）。 */
export async function runMcpServer(): Promise<void> {  // 导出 MCP 服务器运行函数
  ...
  for await (const line of rl) {  // 逐行读取输入（每行一个 JSON-RPC 消息）
    ...
    let request: { jsonrpc?: string; id?: unknown; method?: string; params?: Record<string, unknown> }  // 声明请求对象类型
    try {  // 尝试解析 JSON
      request = JSON.parse(line) as typeof request  // 解析 JSON 行
    } catch {  // 解析失败时
    ...
    try {  // 开始按方法分发处理
      switch (method) {  // 按方法名分发
        ...
        case 'tools/call': {  // 调用工具
          ...
          const toolArgs = (params.arguments ?? {}) as Record<string, unknown>  // 取工具参数，缺省空对象
          let text: string  // 声明返回文本变量
          if (name === 'check_project') text = await checkProject(toolArgs)  // 调用项目检查工具
          else if (name === 'detect_project') text = await detectProject(toolArgs)  // 调用项目识别工具
          else text = '未知工具: ' + name  // 未知工具返回提示
          send({  // 返回工具调用结果
            jsonrpc: '2.0', id,  // 协议版本与请求 id
