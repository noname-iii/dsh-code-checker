    ...
    for (let i = 0; i < argv.length; i++) { // 遍历所有命令行参数
        ...
        switch (arg) { // 根据当前参数匹配各个选项
            ...
            case '--no-llm':
                ...
                break; // 关闭 LLM 深度分析
            case '--build-timeout':
                args.buildTimeoutMs = Number(next()) || args.buildTimeoutMs;
                break; // 设置构建超时（非法值则保留默认）
            case '--probe':
                args.runProbeMs = Number(next()) || args.runProbeMs;
                break; // 设置探针时长（非法值则保留默认）
            case '--artifacts':
                args.artifacts = next();
                break; // 指定模拟产物目录
...
async function main() {
    ...
    if (command === 'detect') { // 请求识别项目类型
        ...
        const io = { exec: makeExec(process.platform === 'win32' ? 'win32' : 'posix'), platform: process.platform === 'win32' ? 'win32' : 'posix', log: (line) => console.error('[detect] ' + line) }; // 构造 IO：执行器、平台与日志函数
        const info = await detectProject(dir, io); // 执行项目识别
        console.log(JSON.stringify(info, null, 2)); // 以缩进 JSON 输出识别结果
        return; // 结束主函数
    }
    if (command !== 'check')
        usage(2); // 命令不是 check 时按用法错误退出
    const args = parseArgs(argv.slice(1)); // 解析 check 命令后面的选项
    if (!args.projectDir)
        usage(2); // 未指定项目目录时按用法错误退出
    ...
    const platform = process.platform === 'win32' ? 'win32' : 'posix'; // 根据运行平台判定为 win32 或 posix
    ...
    if (args.useLlm && args.llmBaseUrl && args.llmModel) { // 启用 LLM 且接口地址与模型齐全时
        ...
        if (apiKey.startsWith('env:'))
            apiKey = process.env[apiKey.slice(4)] ?? ''; // 以 env: 前缀时改为读取对应环境变量
    ...
    const io = {
        ...
        log: (line) => console.error('[check] ' + line), // 日志函数：输出到标准错误
    };
