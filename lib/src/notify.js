...
export function sendSystemNotification(platform, title, body) {
...
export function toolArgumentsFromSession(session, callId) {
    if (callId === undefined || callId === null)
        return undefined; // 无 callId 直接返回
    for (const event of [...session.events].reverse()) { // 倒序遍历会话事件（找最近的匹配）
        if (event.type !== 'tool/call')
            continue; // 跳过非工具调用事件
        const data = event.data; // 取事件数据
        if (data.callId !== callId)
            continue; // callId 不匹配则继续
        const raw = data.arguments ?? ''; // 原始参数 JSON 字符串
        if (!raw)
            return undefined; // 空则返回
        try { // 尝试解析 JSON 提取更友好的命令字段
            const parsed = JSON.parse(raw); // 解析参数
            if (typeof parsed === 'string')
                return parsed.slice(0, 240); // 直接是字符串
            if (parsed && typeof parsed === 'object') { // 对象时优先取常见命令字段
                const obj = parsed; // 转记录类型
                const picked = obj.command ?? obj.cmd ?? obj.input ?? obj.text ?? obj.code ?? obj.path; // 依次取常见字段
                if (typeof picked === 'string' && picked)
                    return picked.slice(0, 240); // 命中字符串字段
...
export function sessionLabel(session) {
    for (const event of [...session.events].reverse()) { // 倒序找会话标题事件（session/title 是 merge 扩展事件，按字符串比较）
        if (event.type === 'session/title') { // 命中标题事件
            const title = event.data.title; // 取标题字段
            if (typeof title === 'string' && title.trim())
                return title.trim().slice(0, 80); // 非空则返回（截断）
            break; // 找到标题事件但没有可用标题，跳出
        }
    }
    const cwd = session.header?.cwd; // 取会话工作目录
    if (cwd) { // 有 cwd
        const name = basename(cwd); // 取目录名
        if (name)
            return name.slice(0, 80); // 返回目录名
    }
    return String(session.id).slice(0, 8); // 回退到会话 id 前 8 位
}
/** 组装审批通知正文（纯函数，便于测试）：会话 + 工具 + 具体信息 + 选项。 */
export function buildApprovalBody(label, toolName, info) {
 ...
 */
export function installApprovalNotifier(ctx, config, log) {
    if (!config.enabled)
        return; // 配置关闭则直接返回
    ctx.on('approval/request', (req, next) => {
        try { // 受保护的提取与发送（通知失败不影响审批）
            const session = req.agent?.session; // 取会话（可能不存在）
