...
// 导入回传与报告截断函数
import { deliverToAgent, truncateReport } from './feedback.js';
...
export function apply(ctx, config) {
    ...
    const log = (line) => { console.log('[dsh-code-checker] ' + line); }; // 定义日志函数：加前缀后输出到 stdout
    ...
    const platform = process.platform === 'win32' ? 'win32' : 'posix'; // 判断当前运行平台（win32 或 posix）
    /** 解析项目目录。 */
    const projectDirFor = (session, explicit) => {
        if (explicit && explicit.trim())
            return explicit.trim(); // 有显式目录且非空则去除首尾空白后直接返回
    ...
    const buildAnalyzer = (agent) => {
        if (!config.useLlm)
            return undefined; // 未启用 LLM 则直接返回 undefined
    ...
    agent, // 参数：目标 agent（可能不存在）
    reason, // 参数：触发方式（自动/命令/工具）
    options = {}) => {
        ...
        if (agent && reason !== 'tool') { // 仅当有 agent 且不是工具触发时才回传
            const message = report.ok // 报告是否干净（无问题）
                ...
                : truncateReport(report.rendered, config.maxReportChars); // 否则截断渲染后的报告文本
            const delivered = deliverToAgent(agent, message, config.reportToAi); // 按配置方式将消息回传给 agent
            log(delivered // 根据回传结果输出日志
                ? (report.ok ? '检查通过，已回传“' + config.cleanMessage + '”。' : '发现问题，已回传报告给 AI。') // 回传成功：区分通过/发现问题
    ...
    installTracker(ctx, {
        ...
        async runCheckForAgent(agent, reason, extraRequirements, signal) {
    ...
    // ── /check 命令 ──
    installCommand(ctx, {
        async runCheckForAgent(agent, _reason, extraRequirements) {
            await runForAgent(agent, 'command', { extraRequirements }); // 以 command 方式执行检查
        },
        lastReportText(sessionId) {
    ...
    // ── check_project 模型工具 ──
    installTool(ctx, {
        async runCheckReturnText(agent, _reason, options) {
