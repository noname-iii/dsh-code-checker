...
function webDefaultPlan(panelPath) {
    return {
        kind: 'web', // web 类型
        interactions: [
...
project) {
    ...
    if (io.analyzer && opts.useLlm && sourceText.trim()) { // 有分析器、允许 LLM 且有来源文本时
        try {
            ...
            if (start >= 0 && end > start) { // 找到有效 JSON 范围时
                const plan = JSON.parse(raw.slice(start, end + 1)); // 解析 JSON
                if (typeof plan === 'object' && plan !== null && typeof plan.kind === 'string') { // 解析结果有效时
                    io.log('[第3步] 使用 LLM 生成的模拟计划: kind=' + plan.kind); // 输出日志
                    return {
                        kind: ['web', 'cli', 'desktop', 'none'].includes(plan.kind) ? plan.kind : 'none', // 校验 kind 取值
    ...
    switch (projectInfo.kind) { // 按项目类型选择默认计划
        ...
        default: { // 其他类型：先判断 GUI，再回退 CLI
            ...
            if (gui === 'web')
                return webDefaultPlan(guiPanelPaths(project)[0]); // GUI（web 类，如 DSH 插件面板）→ web 模拟（直接访问面板路径）
            if (gui === 'desktop')
                return desktopDefaultPlan(); // GUI（桌面框架）→ 桌面模拟
            return cliDefaultPlan(); // 无 GUI → CLI 模拟
        }
    }
}
/** HTTP 探针：检测 web 服务是否存活/卡顿（node:http 实现，避免 undici 长连接在进程退出时崩溃）。 */
async function httpProbe(url, io) {
...
trace) {
                ...
                try {
                    ...
                    if (result.playwright) { // Playwright 可用
                        ...
                        for (const bc of buttonClicks) { // 逐个按钮检查
                            if (bc.skipped)
                                continue; // 禁用/隐藏按钮未真正点击，跳过（不记录异常，也不当作真实点击）
    ...
    if (server) { // 若启动了服务器
        ...
        catch { /* 忽略停止失败 */ } // 停止服务器（忽略失败）
    }
    if (trace && baseUrl)
        trace.baseUrl = baseUrl; // 记录探测/启动到的服务地址（供 GUI 展示）
...
opts, // 检查配置
io, // IO 适配器
project, // 项目文件采样（GUI 判定用；缺省时按项目类型推断）
trace) {
    const started = Date.now(); // 记录开始时间
    const detail = []; // 累积详细说明
    const findings = []; // 累积问题
    ...
    if (gui === 'web' && plan.kind !== 'web') { // GUI（web 类）但计划不是 web → 强制替换
        ...
        Object.assign(plan, webDefaultPlan(guiPanelPaths(project)[0])); // 替换为 web 默认计划（优先面板路径）
    }
    ...
    if (trace)
        trace.screenshots = produced; // 记录截图产物（供 GUI 展示）
    ...
    if (status === 'passed')
        detail.push(testResult.ran ? '未发现卡顿、无响应、报错等异常，自动化测试全部通过。' : '未发现卡顿、无响应、报错等异常。'); // 通过时记录说明
