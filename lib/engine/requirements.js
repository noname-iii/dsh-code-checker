...
function looksLikeRequirement(text) {
    ...
    // 含行为动词或特征词
    if (/实现|开发|编写|创建|添加|增加|支持|提供|展示|显示|允许|需要|要求|包含|具备|能够|可以|应该|设计|制作|生成|绘制|渲染|接入|对接|集成|部署|构建|打包|测试|make|build|create|add|support|implement|show|display|render|include|provide|allow|require|should|must|need|develop|write|design/i.test(t)) { // 若包含行为动词或特征词
        return true; // 判定为需求
    }
