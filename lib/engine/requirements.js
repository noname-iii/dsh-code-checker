...
export function extractTerms(text) {
    ...
    if (hasCjk(text)) { // 若文本包含中文
        ...
        for (let i = 0; i + 1 < joined.length; i++) { // 遍历生成相邻双字符组合
            ...
            if (!terms.includes(bigram))
                terms.push(bigram); // 去重后加入关键词
        }
    }
    ...
    for (const w of words)
        if (!terms.includes(w))
            terms.push(w); // 将去重后的英文单词加入关键词
    return terms.slice(0, 40); // 最多返回前 40 个关键词
}
/** 判断文本是否像终端命令行 / shell 提示符片段（这类不是功能需求，需过滤掉）。 */
function looksLikeShellCommand(text) {
    const t = text.trim(); // 去除首尾空白
    if (!t)
        return false; // 空串不是命令
    if (/^[^\s@]+@[^\s:]+:\S*[$#]\s+/.test(t))
        return true; // bash: user@host:~/dir$ 命令
    if (/^[^\s@]+@[^\s]+\s+\S+\s*[%#]\s+/.test(t))
        return true; // zsh: user@host dir % 命令
    if (/^(PS\s+)?[A-Za-z]:\\[^>]*>\s*/.test(t))
        return true; // Windows 提示符: C:\dir> 或 PS C:\dir>
    if (/^[$#>%]\s+/.test(t))
        return true; // 裸提示符: $ / # / > / %
    return false; // 否则不是命令
}
/** 判断一句话是否像需求描述（过滤纯寒暄/询问与终端命令行）。 */
function looksLikeRequirement(text) {
    const t = text.trim(); // 去除首尾空白
    if (t.length < 4 || t.length > 600)
        return false; // 长度过短或过长都不视为需求
    if (looksLikeShellCommand(t))
        return false; // 终端命令行 / shell 提示符不是功能需求
    // 过滤纯提问寒暄
    if (/^(你好|您好|hi|hello|hey|谢谢|thanks|ok|好的|明白|收到)[，。!！?？\s]*$/i.test(t))
        return false; // 纯寒暄 / 致谢语句不视为需求
    // 过滤“功能列表引导句”：例如“请帮我增加以下功能：”——它只是引出下面的编号条目，本身不是一条功能。
    if (/^(?:请|麻烦|帮我|请帮我).{0,8}(?:增加|添加|实现|加|做)(?:以下|如下)?(?:这些|这个|几个)?功能[：:]\s*$/.test(t))
        return false; // 功能列表引导句
    // 过滤流程元指令：让 AI“做完后确保没问题再汇报/报告”之类的话不是功能需求。
    if (/请确保.{0,30}(?:没有问题|没问题).{0,60}(?:再汇报|汇报|再报告|报告)/.test(t))
        return false; // 流程元指令（做完再汇报）
    ...
    if (questionWord.test(t) && /[？?吗呢]$/.test(t) && !demandVerb.test(t))
        return false; // 纯提问句（疑问词 + 疑问结尾 + 无需求动词）→ 不是需求
    // 含行为动词或特征词
 ...
 */
export function extractRequirements(text, maxItems = 200) {
    if (!text)
        return []; // 空文本直接返回空数组
    // 按行切，行内再按中英文句末标点切。
    // 注意：(?!\d) 防止把“v0.2.2”“3.14”等数字中的小数点当成句末标点切开。
    const raw = []; // 初始化按句切分的结果数组
    for (const line of text.split(/\r?\n/)) { // 按换行符切分文本并遍历每一行
        const pieces = line // 对当前行进行切分
            .split(/(?<=[。！？；!?;])|(?<=\.)(?=\s|$)/) // 按中英文句末标点切分（保留标点）；英文句点仅在后跟空白/行尾时切，避免切开 .ts/.js/.com 等文件名与域名
    ...
    const seen = new Set(); // 用于去重的已见集合
    const out = []; // 最终需求条目数组
    const push = (item) => {
        const t = item.replace(/^\s*(?:[-*•·]|\d+[.、)])\s*/, '').trim(); // 去除行首列表符号 / 编号并修剪空白
        if (!t || seen.has(t))
            return; // 空内容或已存在则跳过
        if (!looksLikeRequirement(t))
            return; // 不像需求描述则跳过
        seen.add(t); // 记录已见
        out.push(t); // 加入结果数组
    };
