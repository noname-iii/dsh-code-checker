...
export async function runCheck(opts, io) {
    ...
    const trace = { logs: [], commands: [], operations: [], simKind: 'none', hasGui: false, screenshots: [] }; // 追踪数据容器
    const tail = (s, n = 8000) => s.length > n ? '…（输出过长，已截断）\n' + s.slice(-n) : s; // 截断长输出，避免追踪数据过大
    const startFn = io.start; // 先取出后台启动函数，便于闭包内收窄类型
    const tracedIo = {
        ...io, // 原样保留 analyzer / signal / platform 等字段
        log: (line) => { trace.logs.push(line); io.log(line); }, // 记录日志并继续输出
        exec: async (execOpts) => {
        ...
        ...startFn !== undefined ? { start: async (startOpts) => {
