...
export async function detectProject(dir, io) {
    ...
    if (pkg && (base.kind === 'node' || base.kind === 'node-web' || base.kind === 'electron')) { // Node 系项目
        ...
        if (usePytest) { // 有 pytest 证据
            ...
            if (hasTests)
                base.testCommands.push(pyBin + ' -m unittest discover -v');
        }
    }
    else if (base.kind === 'java') { // Java：mvn test / gradle test
        if (await exists(dir, 'build.gradle') || await exists(dir, 'build.gradle.kts'))
            base.testCommands.push('gradle test --console=plain');
        else
            base.testCommands.push('mvn -q test');
    }
        ...
        if (hasTestProject)
            base.testCommands.push('dotnet test');
    }
        ...
        if (await exists(dir, 'CMakeLists.txt')) {
            try {
                ...
                if (/enable_testing|add_test/i.test(cmakeText))
                    base.testCommands.push('ctest --output-on-failure');
            }
            catch { /* 忽略读取失败 */ }
        }
        else {
            try {
                ...
                if (/^\s*(test|check)\s*:/m.test(makeText))
                    base.testCommands.push('make test');
            }
            catch { /* 忽略读取失败 */ }
        }
    }
