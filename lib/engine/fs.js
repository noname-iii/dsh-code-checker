...
export async function scanProject(dir, // 要遍历的根目录
skipDirs = DEFAULT_SKIP_DIRS, // 跳过的目录名集合（可覆盖）
maxFiles = 20000) {
    const out = []; // 收集结果
    /** 内部递归函数：遍历 current 目录下所有条目。 */
    async function walk(current, depth) {
        if (out.length >= maxFiles || depth > 24)
            return; // 超限或过深则停止
        let entries; // 目录条目
        try {
...
function sampleRank(rel) {
    ...
    if (lower.startsWith('src/') || lower.startsWith('app/'))
        rank += 10;
    else if (lower.startsWith('engine/') || lower.startsWith('cli/') || lower.startsWith('lib/src/'))
        rank += 6;
    else if (lower.startsWith('lib/'))
        rank -= 6; // 其余 lib 内容多为编译副本，靠后
    // 测试与示例目录减分（仍有价值，但在实现源码之后）
    if (/(^|\/)(tests?|__tests__|try_it_out|examples|spec|fixtures)\//.test(lower))
        rank -= 4;
    return rank;
}
...
export async function sampleFiles(files, // 待采样的文件清单
maxFiles, // 采样文件数上限
maxBytes, // 采样总字节上限
maxSingleFile = 120_000) {
    ...
    const samples = new Map(); // 采样结果：路径 → 内容
    const seenHashes = new Set(); // 已采样内容哈希（跳过内容完全相同的副本）
    ...
    for (const file of sorted.slice(0, maxFiles)) { // 逐个采样（受文件数上限）
        ...
        try {
            ...
            if (seenHashes.has(hash))
                continue; // 与已采样文件内容完全相同（编译副本）→ 跳过，省预算
...
export function hasFile(relSet, rel) {
...
export function stripTrailingComment(line) {
    let quote = null; // 当前字符串引号（null=不在字符串内）
    ...
    for (let i = 0; i < line.length; i++) { // 逐字符扫描
        ...
        if (ch === '/' && line[i + 1] === '/')
            return line.slice(0, i); // 行尾注释开始，截断
        if (ch === '#')
            return line.slice(0, i); // # 注释开始，截断
    }
 ...
 */
export function searchSamples(samples, needle) {
    const hits = []; // 命中列表
    const lower = needle.toLowerCase(); // 小写化后的关键词
    if (!lower)
        return hits; // 空关键词直接返回
