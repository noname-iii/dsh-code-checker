...
/** 注册 /check 命令。 */
// 导出安装函数：注册 /check 命令
export function installCommand(ctx: Context, deps: CommandDeps): void {
  // 在上下文中注册命令
  ctx.commands.register({
    ...
    async handler(invocation) {
      ...
      if (raw) {
        ...
        if (pathLike && rest) extraRequirements = rest
        // 首词不像路径时，整段输入作为附加需求
        else if (!pathLike) extraRequirements = raw
      }
