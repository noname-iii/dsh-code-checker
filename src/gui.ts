...
import type { CheckReport, CheckTrace } from '../engine/types.js'
...
function emptyTrace(): CheckTrace {
  return { logs: [], commands: [], operations: [], simKind: 'none', hasGui: false, screenshots: [] }
}

/** 去掉报告里的 trace（详情接口不携带大体积追踪数据，trace 走专用端点）。 */
function reportWithoutTrace(report: CheckReport): Omit<CheckReport, 'trace'> {
  const { trace: _trace, ...rest } = report
  return rest
}

/** 合并“正在修改的项目”与“有报告的项目”，得到画面视图左侧项目列表。 */
function buildProjects(store: ReportStore, listProjects?: () => ActiveProject[]): {
  dir: string; name: string; active: boolean; sessionId?: string; lastTime: string | null; ok: boolean | null; reportId: string | null
}[] {
  // dir → 项目行（先放活跃项目，再用报告补充/更新）
  const byDir = new Map<string, { dir: string; name: string; active: boolean; sessionId?: string; lastTime: string | null; ok: boolean | null; reportId: string | null }>()
  ...
  for (const item of store.list()) {
    ...
    if (!existing) {
      ...
      if (!existing.sessionId) existing.sessionId = item.sessionId
    }
  }
