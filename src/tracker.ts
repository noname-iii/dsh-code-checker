...
interface SessionState { // 单个会话的跟踪状态结构
  /** 自上一个用户消息以来的自动检查次数（防循环计数）。 */
  autoChecksSinceUser: number // 自上一个用户消息以来的自动检查次数
  /** 当前轮次的编码活动工具调用数。 */
  codingCallsInTurn: number // 当前轮次的编码活动工具调用计数
  /** 自上一个用户消息以来的编码活动工具调用总数（单调递增）。 */
  codingCallsSeen: number // 自上一个用户消息以来的编码活动调用总数
  /** 上次检查完成时 codingCallsSeen 的值（用于判断是否有“新”编码活动）。 */
  checkedUpTo: number // 上次检查完成时的编码活动计数
  /** 是否正在执行检查（防重入标记）。 */
...
export function installTracker(ctx: Context, deps: TrackerDeps): void { // 导出函数：安装会话跟踪器
  ...
  ctx.on('session/event', (session: Session, event: SessionEvent) => { // 监听会话事件流
    ...
    switch (event.type) { // 按事件类型分发处理
      case 'user/message': { // 用户消息事件
        if (event.data.source.kind !== 'user') break // 非真实用户来源则忽略该事件（插件 steer/inject 的消息不会重置计数）
  ...
  ctx.on('agent/turn-stopping', async (payload) => { // 监听 agent 轮次即将关闭事件
    ...
    if (state.codingCallsInTurn < deps.config.minCodingCalls) return // 本轮编码活动次数不足则不触发检查
    if (state.codingCallsSeen <= state.checkedUpTo) return // 自上次检查后没有新的编码活动则跳过（避免无进展的空转）
