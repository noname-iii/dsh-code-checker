...
export interface Config {
  ...
  notifyApprovals: boolean // 是否发送审批系统通知
}
...
export const DEFAULT_CONFIG: Config = {
  ...
  notifyApprovals: true, // 默认开启：会话需要用户操作时发系统桌面通知
}
  ...
  '~standard': {
    ...
    validate(input: unknown) { // 校验入口：input 为行 config（可能为 undefined）
      ...
      const value: Config = { // 逐字段合并默认值 + 宽松校验
        ...
        notifyApprovals: pickBoolean(source.notifyApprovals, DEFAULT_CONFIG.notifyApprovals), // 是否审批系统通知
      }
      return { value } // 返回校验结果
    },
  },
