...
| **第 3 步 真实用户模拟** | 按用户描述的功能（或 README.md）模拟真实用户：Web 用 Playwright 点击/输入/拖拽/按键；Windows 桌面程序用 UIA 真实鼠标键盘 + 卡死检测 + 截图；CLI 用命令驱动；记录**卡顿、无响应、报错、崩溃** | 有异常 → 全部记录并汇报给 AI；无异常 → 返回 **“没有问题”** 并让 AI 继续工作 |
...
### 方式 1：从 GitHub 克隆（推荐给想自己发布/改代码的你）
    ...
    git clone <你的仓库地址> dsh-code-checker
    cd dsh-code-checker
...
### 方式 3：GitHub 直装（免 clone）
    ...
    dsh plugin --profile web add github:你/dsh-code-checker
