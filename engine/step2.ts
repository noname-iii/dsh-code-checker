...
function stripHtml(html: string): string { // 去掉 HTML 标签/脚本/样式，解码常见实体
...
function fetchPageText(url: string, timeoutMs = 8000): Promise<string> { // 抓取页面文本
  return new Promise((resolve) => { // 用 Promise 包装请求
    let settled = false // 是否已结束
    const done = (s: string): void => { if (!settled) { settled = true; resolve(s) } } // 结束并返回文本
    const req = httpRequest(url, { method: 'GET', headers: { connection: 'close' }, timeout: timeoutMs }, (res) => { // 发起 GET
      let body = '' // 累积响应体
      res.setEncoding('utf8') // 按 utf8 解码
      res.on('data', (c: string) => { body += c; if (body.length > 200000) req.destroy() }) // 累积（超 200KB 截断）
...
async function renderWebText(baseUrl: string, opts: CheckOptions, io: EngineIo): Promise<string> { // Playwright 渲染页面文本
  ...
  try { // 写最小计划并执行
    ...
    if (jsonLine) { // 有结果
      const result = JSON.parse(jsonLine) as { playwright?: boolean; pageText?: string } // 解析
      if (result.playwright && result.pageText) { io.log('[第2步] 行为验证：Playwright 渲染页面拿到可见文本（' + String(result.pageText.length) + ' 字符）。'); return result.pageText } // 成功
    }
  } catch { /* 忽略 Playwright 失败 */ } finally { // 清理临时计划文件
...
async function probeWebText(baseUrl: string, opts: CheckOptions, io: EngineIo): Promise<string> { // 探测并抓取页面可见文本
 ...
 */
async function behavioralText(projectInfo: ProjectInfo, opts: CheckOptions, io: EngineIo): Promise<string> { // 行为验证：取得运行后可见文本
  ...
  if (isWeb) { // web 项目
    for (const port of BEHAVIOR_PORTS) { // 逐个候选端口探测已有服务
      ...
      if (text) { io.log('[第2步] 行为验证：在端口 ' + String(port) + ' 探测到页面（' + String(text.length) + ' 字符可见文本）。'); return text } // 命中即返回
    }
    if (io.start && projectInfo.runCommands[0]) { // 无已有服务且可后台启动时，启动 dev server 再抓取
      let server: Awaited<ReturnType<NonNullable<EngineIo['start']>>> | undefined // 服务器句柄
      try { // 启动并轮询
        ...
        for (let i = 0; i < 20; i++) { // 最多 10 秒轮询
          for (const port of BEHAVIOR_PORTS) { // 逐个端口探测
            const text = await probeWebText('http://127.0.0.1:' + String(port), opts, io) // 探测并抓取页面可见文本
            if (text) { io.log('[第2步] 行为验证：启动后从端口 ' + String(port) + ' 抓取到页面文本。'); return text } // 命中即返回
          }
          await new Promise(resolve => setTimeout(resolve, 500)) // 等 500ms 再试
        }
      } catch { /* 忽略启动/抓取失败 */ } finally { // 无论如何停止服务器
        if (server) { try { await server.stop() } catch { /* 忽略停止失败 */ } }
      }
    }
    return '' // 无法取得页面文本
  }
  // ── CLI：跑 --help / --version 看输出 ──
  const parts: string[] = [] // 累积命令输出
  ...
  if (base) { // 有运行命令
    for (const arg of ['--help', '--version']) { // 两个探针参数
      ...
      try { // 执行命令
        ...
        if (out) parts.push(out.slice(0, 4000)) // 截断后加入
      } catch { /* 忽略执行失败 */ }
    }
  }
  return parts.join('\n') // 返回合并文本
...
export async function runStep2( // 定义第 2 步主函数
  ...
  if (renderedText) detail.push('已模拟打开项目进行行为验证（可见文本 ' + String(renderedText.length) + ' 字符）。') // 记录行为验证
  ...
  for (let i = 0; i < requirements.length; i++) { // 逐条需求核对
    ...
    if (renderedText) { // 有行为验证文本
      ...
      if (hit) { // 运行时可见该功能痕迹
        ...
        if (status === 'missing') { status = 'implemented'; evidence = behavioralNote + '；' + evidence } // 缺失 → 救回为已实现
        else { evidence = behavioralNote + '；' + evidence } // 其它状态追加行为证据
      }
    }
