...
async function llmCheck( // 定义 LLM 深度核对函数
  ...
  try {
    ...
    const prompt = [ // 组装用户提示词
      ...
      '{"index": 编号, "status": "implemented|partial|missing", "evidence": "判断依据（引用具体文件）", "suggestion": "缺失或部分实现时的修复建议（implemented 时可为空字符串）"}', // JSON 元素模板
    ...
    for (const item of parsed) { // 逐个元素解析
      ...
      if (status !== 'implemented' && status !== 'partial' && status !== 'missing') continue // 状态非法则跳过
      verdicts.push({ // 组装并收集合法结论
        index, // 需求编号
        text: requirements[index] ?? '', // 需求文本
        status, // 状态
        evidence: typeof rec.evidence === 'string' ? rec.evidence.slice(0, 500) : '', // 依据（截断 500）
