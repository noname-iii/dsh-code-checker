...
function looksLikeShellCommand(text: string): boolean { // 判断是否像 shell 命令/提示符
  const t = text.trim() // 去除首尾空白
  if (!t) return false // 空串不是命令
  if (/^[^\s@]+@[^\s:]+:\S*[$#]\s+/.test(t)) return true // bash: user@host:~/dir$ 命令
  if (/^[^\s@]+@[^\s]+\s+\S+\s*[%#]\s+/.test(t)) return true // zsh: user@host dir % 命令
  if (/^(PS\s+)?[A-Za-z]:\\[^>]*>\s*/.test(t)) return true // Windows 提示符: C:\dir> 或 PS C:\dir>
  if (/^[$#>%]\s+/.test(t)) return true // 裸提示符: $ / # / > / %
  return false // 否则不是命令
}
...
function looksLikeRequirement(text: string): boolean { // 判断一句话是否像需求描述
  ...
  if (looksLikeShellCommand(t)) return false // 终端命令行 / shell 提示符不是功能需求
  // 过滤纯提问寒暄
  ...
  if (/^(?:请|麻烦|帮我|请帮我).{0,8}(?:增加|添加|实现|加|做)(?:以下|如下)?(?:这些|这个|几个)?功能[：:]\s*$/.test(t)) return false // 功能列表引导句
  // 过滤流程元指令：让 AI“做完后确保没问题再汇报/报告”之类的话不是功能需求。
  if (/请确保.{0,30}(?:没有问题|没问题).{0,60}(?:再汇报|汇报|再报告|报告)/.test(t)) return false // 流程元指令（做完再汇报）
