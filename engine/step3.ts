 ...
 *   web 模拟额外内置：逐页审计（每个页面检查是否卡在某个页面/卡在“加载中”）+
 *   遍历点击页面上所有按钮（逐个点击并记录每个按钮点击后的异样）。
 *
...
// 引入 Node 内置模块：文件系统（promise 版 + 同步版）、路径处理、URL 转路径、随机 UUID
...
import type { ProjectFiles } from './fs.js' // 引入项目文件采样类型
...
/** 定位 simulators 目录：从当前模块目录向上查找（兼容 lib/engine 编译产物与源码直跑两种层级）。 */
function findSimulatorsDir(): string { // 向上查找 simulators 目录
  let dir = moduleDir // 从当前模块目录开始
  for (let i = 0; i < 4; i++) { // 最多向上 4 层
    const candidate = join(dir, 'simulators') // 当前层的 simulators 候选路径
    if (existsSync(candidate)) return candidate // 存在即返回
    const parent = dirname(dir) // 上一层
    if (parent === dir) break // 到根目录则停止
    dir = parent // 继续向上
  }
  return join(moduleDir, '..', 'simulators') // 兜底：模块上级目录的 simulators
}
const SIMULATORS_DIR = findSimulatorsDir() // 模拟器脚本目录（自动向上定位，编译产物与源码直跑都可用）

/** 构造默认 web 模拟计划（可选面板路径，DSH 插件面板等 GUI 项目使用）。 */
function webDefaultPlan(panelPath?: string): SimPlan { // web 默认计划工厂
  return { // 返回 web 默认计划
    kind: 'web', // web 类型
    interactions: [ // 默认交互序列
      { action: 'goto', target: panelPath ?? '/' }, // 访问面板（或首页）
    ...
    commands: [], // web 无命令行
    ports: [3080, 5173, 3000, 8080, 4173, 5000, 8000], // 候选端口（3080 为 DeepSeek Harness Web 面板端口）
  }
}

/** 构造默认桌面模拟计划（Windows UIA）。 */
function desktopDefaultPlan(): SimPlan { // 桌面默认计划工厂
  return { // 返回桌面默认计划
    kind: 'desktop', // 桌面类型
    interactions: [ // 默认交互序列
...
/** 构造默认 CLI 模拟计划。 */
function cliDefaultPlan(): SimPlan { // CLI 默认计划工厂
  return { // 返回 cli 默认计划
    kind: 'cli', // cli 类型
...
async function buildPlan( // 定义模拟计划生成函数
  ...
  // 默认计划（按项目类型；带 GUI 的项目强制走 GUI 模拟）
  switch (projectInfo.kind) { // 按项目类型选择默认计划
    case 'web-static': // 静态 Web 项目
    case 'node-web': // Node Web 项目
      return webDefaultPlan() // 返回 web 默认计划
    case 'electron': // Electron 应用
    case 'desktop-exe': // Windows 桌面程序
      return desktopDefaultPlan() // 返回桌面默认计划
    default: { // 其他类型：先判断 GUI，再回退 CLI
      const gui = guiSimKind(project, projectInfo) // 判定 GUI 模拟类型
      if (gui === 'web') return webDefaultPlan(guiPanelPaths(project)[0]) // GUI（web 类，如 DSH 插件面板）→ web 模拟（直接访问面板路径）
      if (gui === 'desktop') return desktopDefaultPlan() // GUI（桌面框架）→ 桌面模拟
      return cliDefaultPlan() // 无 GUI → CLI 模拟
    }
  }
}

/** HTTP 探针：检测 web 服务是否存活/卡顿（node:http 实现，避免 undici 长连接在进程退出时崩溃）。 */
async function httpProbe(url: string, io: EngineIo): Promise<{ ok: boolean; status?: number; durationMs: number; error?: string }> { // 定义 HTTP 探针函数
  ...
  return await new Promise((resolve) => { // 用 Promise 包装请求生命周期
    let settled = false // 是否已结束（防止重复 resolve）
    const done = (result: { ok: boolean; status?: number; durationMs: number; error?: string }): void => { // 结束函数
      if (settled) return // 已结束则忽略
      settled = true // 标记已结束
      resolve(result) // 返回结果
    }
    const req = httpRequest(url, { // 发起 GET 请求
      method: 'GET', // GET 方法
      headers: { connection: 'close' }, // 关闭长连接（探测完即断，不留 undici 式 keep-alive 句柄）
      timeout: 6000, // 6 秒超时
    }, (res) => { // 响应回调
      const durationMs = Date.now() - started // 计算耗时
      const status = res.statusCode // 状态码
      res.resume() // 丢弃响应体
      res.on('end', () => done({ ok: status !== undefined && status >= 200 && status < 500, status, durationMs })) // 响应结束后返回结果（2xx~4xx 视为存活）
      res.on('error', (error) => done({ ok: false, durationMs: Date.now() - started, error: error instanceof Error ? error.message : String(error) })) // 响应流异常
    })
    req.on('timeout', () => { req.destroy(new Error('探测超时')) }) // 超时销毁请求
    req.on('error', (error) => done({ ok: false, durationMs: Date.now() - started, error: error instanceof Error ? error.message : String(error) })) // 请求异常
    req.end() // 发送请求
  })
}
...
async function simulateWeb( // 定义 web 模拟函数
  ...
  if (baseUrl) { // 有服务地址时才做浏览器自动化
    ...
    await fsp.writeFile(planFile, JSON.stringify({ baseUrl, interactions: plan.interactions, artifacts, audit: true }, null, 2), 'utf8') // 把计划写入文件（audit=true 开启逐页审计 + 点击所有按钮）
      ...
      if (jsonLine) { // 找到结果行
        try {
          const result = JSON.parse(jsonLine) as { // 解析结果 JSON
            ...
            pageChecks?: { url: string; title?: string; ok: boolean; stuckLoading?: boolean; note?: string }[]; // 逐页审计结果
            buttonClicks?: { url: string; text?: string; ok: boolean; skipped?: boolean; error?: string; navigatedTo?: string }[]; // 所有按钮点击结果
          }
          if (result.playwright) { // Playwright 可用
            playwrightUsed = true // 标记使用了 Playwright
            const pageChecks = Array.isArray(result.pageChecks) ? result.pageChecks : [] // 逐页审计（旧脚本无此字段则空）
            const buttonClicks = Array.isArray(result.buttonClicks) ? result.buttonClicks : [] // 按钮点击结果（旧脚本无此字段则空）
            ...
            // 逐页审计：每个页面检查“卡在加载中 / 加载失败”等异样
            for (const pc of pageChecks) { // 逐个页面检查
              trace?.operations.push({ kind: 'web', action: 'page-audit', target: pc.url, ok: pc.ok, error: pc.note || undefined }) // 记录页面审计操作（供 GUI 展示）
              if (!pc.ok) { // 页面审计发现异样
                anomalies.push({ // 记录异常
                  kind: pc.stuckLoading ? 'freeze' : 'error', // 卡“加载中”→卡顿；加载失败→报错
                  where: pc.stuckLoading ? '页面卡在加载中: ' + pc.url : '页面加载异常: ' + pc.url, // 位置
                  message: pc.note || '页面存在异样', // 消息
                  evidence: pc.note, // 证据
                })
              }
            }
            // 模拟点击所有按钮：逐个点击并记录异样（报错 / 点击后卡在加载中）
            for (const bc of buttonClicks) { // 逐个按钮检查
              if (bc.skipped) continue // 禁用/隐藏按钮未真正点击，跳过（不记录异常，也不当作真实点击）
              trace?.operations.push({ kind: 'web', action: 'click-button', target: bc.text || undefined, ok: bc.ok, error: bc.error || undefined }) // 记录按钮点击操作（供 GUI 展示）
              if (!bc.ok) { // 点击失败
                const timeoutish = /超时|无响应|timeout|加载指示/i.test(bc.error ?? '') // 是否疑似无响应/卡加载
                anomalies.push({ // 记录异常
                  kind: timeoutish ? 'unresponsive' : 'error', // 无响应/卡加载→unresponsive；其余→error
                  where: '按钮点击: ' + (bc.text || bc.url), // 位置
                  message: bc.error ?? '按钮点击未成功', // 消息
                  evidence: bc.navigatedTo ? '点击后跳转到 ' + bc.navigatedTo : undefined, // 证据
                })
              }
            }
            ...
            detail.push('浏览器自动化完成：' + String(result.actions.length) + ' 个操作，逐页审计 ' + String(pageChecks.length) + ' 个页面，点击按钮 ' + String(buttonClicks.length) + ' 次，控制台错误 ' + String(result.consoleErrors.length) + ' 条，页面错误 ' + String(result.pageErrors.length) + ' 条。') // 汇总
          } else { // Playwright 未安装
...
/** 执行项目自动化测试（Node 项目跑 `pnpm test`；非 Node 项目跑等价测试命令）。 */
async function runAutomatedTests( // 定义自动化测试函数
  projectInfo: ProjectInfo, // 项目信息
  opts: CheckOptions, // 检查配置
  io: EngineIo, // IO 适配器
): Promise<{ anomalies: Anomaly[]; detail: string[]; ran: boolean }> { // 返回测试结果
  const anomalies: Anomaly[] = [] // 累积异常
  const detail: string[] = [] // 累积说明
  const commands = projectInfo.testCommands ?? [] // 取检测到的测试命令
  if (commands.length === 0) { // 无测试命令
    detail.push('未检测到自动化测试命令，跳过自动化测试。') // 记录说明
    return { anomalies, detail, ran: false } // 返回未运行
  }
  let ran = false // 是否实际运行过
  for (const command of commands.slice(0, 3)) { // 最多执行 3 条测试命令
    io.log('[第3步] 自动化测试: ' + command) // 输出日志
    const started = Date.now() // 记录开始时间
    const res = await io.exec({ command, cwd: opts.projectDir, timeoutMs: opts.buildTimeoutMs }) // 执行测试命令
    ran = true // 标记已运行
    const durationMs = Date.now() - started // 计算耗时
    if (res.timedOut) { // 测试超时
      anomalies.push({ kind: 'unresponsive', where: '自动化测试: ' + command, message: '自动化测试 ' + String(durationMs) + 'ms 无响应（超时被杀），疑似卡死', durationMs }) // 记录无响应异常
      detail.push('自动化测试超时: ' + command) // 记录说明
      continue // 继续下一条
    }
    if (res.exitCode !== 0 && res.exitCode !== null) { // 非零退出码
      const errorLines = extractErrors(res.stdout, res.stderr, 3) // 提取错误行
      anomalies.push({ // 记录错误异常
        kind: 'error', // 错误级
        where: '自动化测试: ' + command, // 位置
        message: '自动化测试失败（退出码 ' + String(res.exitCode) + '）' + (errorLines.length > 0 ? '：' + errorLines.map(e => e.text).join(' | ') : ''), // 消息
        evidence: (res.stderr || res.stdout).slice(0, 800), // 证据
      })
      detail.push('自动化测试报错: ' + command) // 记录说明
      continue // 继续下一条
    }
    detail.push('自动化测试通过: ' + command) // 记录通过
  }
  return { anomalies, detail, ran } // 返回测试结果
}
...
export async function runStep3( // 定义第 3 步主函数
  ...
  const plan = await buildPlan(sourceText, projectInfo, opts, io, project ?? { files: [], relSet: new Set(), samples: new Map(), samplingTruncated: false, sampledBytes: 0 }) // 推导模拟计划

  // 规则：项目带 GUI（用户操作界面，含 DSH 插件面板）时，第 3 步必须走 GUI 模拟——
  // LLM 计划若给出 cli/none 会被强制替换为对应 GUI 模拟计划。
  const gui = project ? guiSimKind(project, projectInfo) : 'none' // 判定 GUI 模拟类型
  if (gui === 'web' && plan.kind !== 'web') { // GUI（web 类）但计划不是 web → 强制替换
    detail.push('项目带用户界面（GUI）：强制使用 web 模拟计划。') // 记录说明
    Object.assign(plan, webDefaultPlan(guiPanelPaths(project as ProjectFiles)[0])) // 替换为 web 默认计划（优先面板路径）
  } else if (gui === 'desktop' && plan.kind !== 'desktop') { // GUI（桌面类）但计划不是 desktop → 强制替换
    detail.push('项目带用户界面（GUI）：强制使用桌面模拟计划。') // 记录说明
  ...
  // 3.a) 自动化测试（Node 项目 `pnpm test`；非 Node 项目等价测试命令）—— 与用户模拟并列，共同组成第 3 步。
  const testResult = await runAutomatedTests(projectInfo, opts, io) // 执行自动化测试
  detail.push(...testResult.detail) // 记录自动化测试说明
  ...
  anomalies.push(...testResult.anomalies) // 合并自动化测试异常
  anomalies.push(...result.anomalies) // 合并用户模拟异常
  ...
  const ran = result.ran || testResult.ran // 自动化测试或用户模拟任一实际运行过即视为第 3 步已运行
  const status = !ran // 未运行时
    ? 'skipped' // 跳过
    : (anomalies.some(a => a.kind !== 'warning') ? 'failed' : (anomalies.length > 0 ? 'partial' : 'passed')) // 有非警告异常 failed，仅警告 partial，无异常 passed
  if (status === 'passed') detail.push(testResult.ran ? '未发现卡顿、无响应、报错等异常，自动化测试全部通过。' : '未发现卡顿、无响应、报错等异常。') // 通过时记录说明
