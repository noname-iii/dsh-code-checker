...
import type { CheckOptions, EngineIo, ExecResult, Finding, StepResult } from './types.js' // 引入检查选项、引擎 IO、执行结果、发现、步骤结果等类型
...
/** 错误模式：输出中出现即视为报错。 */
const ERROR_PATTERNS: RegExp[] = [ // 定义错误识别正则数组
...
const LOCATION_PATTERNS: RegExp[] = [ // 定位正则数组（按优先级）
...
function locateError(line: string): { file?: string; line?: number; column?: number } { // 解析错误定位
...
export function formatErrorLocation(error: LocatedError): string { // 格式化定位
...
export function extractErrors(stdout: string, stderr: string, max = 8): LocatedError[] { // 从 stdout/stderr 中提取最多 max 条错误
  const combined = [stderr, stdout].filter(Boolean).join('\n') // 将 stderr 与 stdout 去空后合并为一个文本
  const lines = combined.split(/\r?\n/) // 按换行符拆分为行数组
  const errors: LocatedError[] = [] // 初始化错误结果数组
  for (let i = 0; i < lines.length; i++) { // 遍历每一行
    if (errors.length >= max) break // 达到上限则停止提取
    ...
    if (match) { // 若命中某模式
      ...
      const error: LocatedError = { // 组装结构化错误
        text, // 错误文本
      ...
      if (!dup) errors.push(error) // 非重复才加入
    }
  }
...
function renderErrors(errors: LocatedError[], maxItems = 4): string { // 渲染错误列表
...
async function installDeps(project: ProjectInfo, opts: CheckOptions, io: EngineIo): Promise<Finding[]> { // 按条件执行依赖安装并返回发现
  const findings: Finding[] = [] // 初始化发现数组
  if (!opts.installDeps) return findings // 配置不允许安装则直接返回空
  if (!project.packageJson || !project.lockfile) return findings // 无 package.json 或锁文件则不安装
  if (existsSync(join(opts.projectDir, 'node_modules'))) return findings // 已存在 node_modules 则跳过安装
  const npm = io.platform === 'win32' ? 'npm.cmd' : 'npm' // 根据平台选择 npm 命令名
  let command = npm + ' install --no-audit --no-fund' // 默认安装命令
  if (project.lockfile === 'npm') command = npm + ' ci --no-audit --no-fund' // npm 锁文件用 ci 精确安装
  if (project.lockfile === 'pnpm') command = 'pnpm install --frozen-lockfile' // pnpm 用 frozen-lockfile 安装
  if (project.lockfile === 'yarn') command = 'yarn install --frozen-lockfile' // yarn 用 frozen-lockfile 安装
...
async function runOnce( // 定义执行单条命令的辅助函数
  command: string, // 参数：要执行的命令
  cwd: string, // 参数：执行工作目录
  timeoutMs: number, // 参数：超时时间（毫秒）
  stage: string, // 参数：阶段名称（如“构建”）
  io: EngineIo, // 参数：引擎 IO 接口
): Promise<{ res: ExecResult; findings: Finding[] }> { // 执行命令并返回执行结果与发现
  io.log('[第1步] ' + stage + ': ' + command) // 记录执行日志
  const started = Date.now() // 记录开始时间
  let res: ExecResult // 声明执行结果变量
  try { // 尝试执行命令
    res = await io.exec({ command, cwd, timeoutMs }) // 调用 IO 执行命令
  } catch (error) { // 执行抛出异常
    const message = error instanceof Error ? error.message : String(error) // 提取异常消息
    return { // 返回异常时的执行结果与发现
...
async function runProbe( // 定义运行探针辅助函数
  command: string, // 参数：要探测的运行命令
  opts: CheckOptions, // 参数：检查选项
  io: EngineIo, // 参数：引擎 IO 接口
): Promise<{ findings: Finding[]; detail: string[] }> { // 运行探针并返回发现与详情
  const { res, findings } = await runOnce(command, opts.projectDir, opts.runProbeMs, '运行探针', io) // 执行运行探针并解构结果
  const detail: string[] = [] // 初始化详情数组
 ...
 * 执行第 1 步。
 */
export async function runStep1(opts: CheckOptions, io: EngineIo): Promise<StepResult> { // 执行第 1 步检查并返回步骤结果
  const started = Date.now() // 记录开始时间
  const detail: string[] = [] // 初始化详情数组
  const findings: Finding[] = [] // 初始化发现数组
  const project = await detectProject(opts.projectDir, io) // 检测项目类型与命令
  detail.push('项目类型: ' + project.kind) // 记录项目类型
  if (project.name) detail.push('项目名: ' + project.name) // 有项目名则记录
  if (project.readme) detail.push('检测到 README，将在第 3 步作为操作依据。') // 有 README 则记录提示
  ...
  if (findings.filter(f => f.level === 'error').length === 0) { // 构建无 error 才运行探针
    ...
    if (ran) detail.push('运行探针时长: ' + String(opts.runProbeMs) + 'ms') // 记录探针时长
  }
  ...
  if (status === 'passed') detail.push('未发现编译/运行报错。') // 通过则记录提示
  else if (status === 'failed') detail.push('共发现 ' + String(errors.length) + ' 处报错（已全部列出）。') // 失败时注明已全部列出
  return { // 返回步骤结果
    step: 1, // 步骤编号
    title: '编译与运行检查', // 步骤标题
    status, // 状态
