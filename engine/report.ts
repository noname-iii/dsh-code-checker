...
// 从类型定义中引入报告渲染所需的类型（仅在编译期使用）
import type { CheckOptions, CheckReport, StepResult } from './types.js'
...
// 步骤状态对应的中文文案
const STATUS_TEXT: Record<StepResult['status'], string> = {
  passed: '通过', // 通过
  failed: '未通过', // 未通过
  skipped: '跳过', // 跳过
  partial: '部分通过', // 部分通过
...
const KIND_TEXT_ZH: Record<string, string> = {
...
const KIND_TEXT_EN: Record<string, string> = {
...
const STEP_TITLE_EN: Record<number, string> = {
...
/** 渲染单步结果。 */
function renderStep(step: StepResult, lang: 'zh' | 'en'): string {
  ...
  for (const finding of step.findings) { // 逐条输出该步骤发现的问题
    ...
    if (finding.evidence) lines.push('    证据: ' + finding.evidence.split('\n').slice(0, 6).join('\n    ')) // 有证据时输出（最多前 6 行）
  }
  return lines.join('\n') // 把该步骤所有行用换行拼接成一段文本
}

/** 渲染完整报告文本。 */
export function renderReport(report: CheckReport, opts: Pick<CheckOptions, 'language' | 'cleanMessage'>): string {
  const lang = opts.language // 取出报告语言配置（'zh' 或 'en'）
  const kindTable = lang === 'zh' ? KIND_TEXT_ZH : KIND_TEXT_EN // 根据语言选择项目类型名称表
  const lines: string[] = [] // 用于累积报告的每一行输出
