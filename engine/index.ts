...
/** 合并默认配置。 */
export function resolveOptions(partial: Partial<CheckOptions> & { projectDir: string }): CheckOptions {
  return {
    ...DEFAULTS, // 先铺默认值
...
/** 汇总报告结论。 */
function summarize(report: CheckReport, opts: CheckOptions): { ok: boolean; summary: string } {
  ...
  const parts: string[] = [] // 用于累积各类问题的中文/英文描述片段
  // 按步骤统计错误，避免把第 2 步的缺失功能误报成第 1 步的编译错误
  const stepErrorCount = (step: 1 | 2 | 3) => report.steps // 统计某一步的错误级发现数量
  ...
  if (errors1 > 0) parts.push(zh ? '第 1 步（编译/运行）发现 ' + String(errors1) + ' 处报错' : 'Step 1 (build/run): ' + String(errors1) + ' error(s)') // 第 1 步报错
  if (missing.length > 0) parts.push(zh ? '有 ' + String(missing.length) + ' 条需求未实现或不完整' : String(missing.length) + ' requirement(s) missing/partial') // 功能缺失
  if (errors2 > missing.length) parts.push(zh ? '第 2 步（功能核对）另发现 ' + String(errors2 - missing.length) + ' 处错误' : 'Step 2 (completeness): ' + String(errors2 - missing.length) + ' other error(s)') // 第 2 步除缺失外的其他错误
  if (errors3 > 0) parts.push(zh ? '第 3 步（用户模拟）发现 ' + String(errors3) + ' 处报错' : 'Step 3 (simulation): ' + String(errors3) + ' error(s)') // 第 3 步报错
  else if (anomalies.length > 0) parts.push(zh ? '用户模拟发现 ' + String(anomalies.length) + ' 处异常' : String(anomalies.length) + ' anomaly(ies) in simulation') // 模拟异常
  return {
...
export async function runCheck(opts: CheckOptions, io: EngineIo): Promise<CheckReport> {
  ...
  const steps: StepResult[] = [] // 累积每一步的结果
  const issues: Finding[] = [] // 累积所有步骤发现的问题
  ...
  let step2: StepResult | undefined // 第 2 步结果（可能不执行）
  ...
  const report: CheckReport = { // 组装完整检查报告对象
