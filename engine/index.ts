...
function summarize(report: CheckReport, opts: CheckOptions): { ok: boolean; summary: string } {
  ...
  if (errors.length > 0) parts.push(zh ? '第 1/3 步发现 ' + String(errors.length) + ' 处报错' : String(errors.length) + ' error(s) found') // 编译/运行报错
  ...
  if (anomalies.length > 0) parts.push(zh ? '用户模拟发现 ' + String(anomalies.length) + ' 处异常' : String(anomalies.length) + ' anomaly(ies) in simulation') // 模拟异常
  return {
