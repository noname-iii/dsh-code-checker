...
export async function detectProject(dir: string, io: EngineIo): Promise<ProjectInfo> { // 检测项目类型并推导构建/运行命令
  ...
  // 即使第 2 步发现功能缺失也不跳过。
  const guiDeps = [ // 常见 GUI 框架 / 界面库依赖名
...
export type GuiSimKind = 'web' | 'desktop' | 'none' // GUI 模拟类型三选一
 ...
 */
export function guiSimKind(project: ProjectFiles, info: ProjectInfo): GuiSimKind { // 判定 GUI 模拟类型
  // 第 1 层：项目类型与依赖
  if (info.hasGui) { // 类型/依赖已判定有 GUI
    if (info.kind === 'desktop-exe' || info.kind === 'electron') return 'desktop' // 桌面程序
    return 'web' // 其余（web-static / node-web / GUI 依赖）按 web 模拟
  }
  // 证据：前端界面文件（网页/组件）
  if (project.files.some(f => /\.(vue|jsx|tsx|svelte|html?)$/i.test(f.rel))) return 'web' // 前端源码 → web 模拟
  const content = [...project.samples.values()].join('\n').toLowerCase() // 全部采样内容合并并小写
  // 证据：DeepSeek Harness 插件面板 / Web UI 框架痕迹
  const webMarkers = [ // web 类 GUI 标记
  ...
  if (webMarkers.some(marker => content.includes(marker.toLowerCase()))) return 'web' // 命中 web 标记
  // 证据：桌面 GUI 框架痕迹
  const desktopMarkers = [ // desktop 类 GUI 标记
  ...
  if (desktopMarkers.some(marker => content.includes(marker.toLowerCase()))) return 'desktop' // 命中桌面标记
 ...
 */
export function guiPanelPaths(project: ProjectFiles): string[] { // 提取面板路径列表
  const paths: string[] = [] // 累积路径
  for (const content of project.samples.values()) { // 遍历采样内容
    if (!content.includes('webServer.register') && !content.includes('installGui')) continue // 非面板挂载文件跳过
    const re = /path\s*:\s*['"]([^'"]+)['"]/g // 匹配 path: '...' 字面量
    let match: RegExpExecArray | null // 单次匹配结果
    while ((match = re.exec(content)) !== null) { // 逐个匹配
      const p = match[1] // 捕获的路径
      if (p && /^\/[A-Za-z0-9\-_./]*$/.test(p) && !paths.includes(p)) paths.push(p) // 合法路径且不重复才收集
      if (paths.length >= 4) return paths // 最多 4 个
    }
  }
  return paths // 返回收集结果
}
